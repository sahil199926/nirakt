    ...
    <section className="py-14 md:py-18 bg-sand overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex gap-4">
              {TESTIMONIALS.map((testimonial) => (
                <div key={testimonial.name} className="flex-[0_0_100%] sm:flex-[0_0_calc(50%-8px)] lg:flex-[0_0_calc(33.333%-11px)] min-w-0">
                  <div className="bg-white rounded-2xl p-5 lg:p-6 h-full flex flex-col relative border border-sand">
                    <Quote className="w-6 h-6 text-primary/10 mb-2" />
                    <p className="text-sm text-text-muted leading-relaxed flex-1">&ldquo;{testimonial.quote}&rdquo;</p>
        ...
        <div className="flex items-center justify-center gap-3 mt-6">
          <button onClick={() => emblaApi?.scrollPrev()} className="w-8 h-8 rounded-full border border-text-muted flex items-center justify-center text-text-muted hover:border-primary hover:text-primary transition-colors" aria-label="Previous">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <div className="flex gap-1.5">
            {TESTIMONIALS.map((_, index) => (
              <button key={index} onClick={() => emblaApi?.scrollTo(index)}
                className={cn("h-1.5 rounded-full transition-all", selectedIndex === index ? "bg-primary w-5" : "bg-text-muted w-1.5 hover:bg-text-muted")}
                aria-label={`Go to slide ${index + 1}`}
              />
            ))}
          </div>
          <button onClick={() => emblaApi?.scrollNext()} className="w-8 h-8 rounded-full border border-text-muted flex items-center justify-center text-text-muted hover:border-primary hover:text-primary transition-colors" aria-label="Next">
